...
export default function MasterDataPage() {
  ...
  const openModal = (item: any = null) => {
    ...
    if (activeTab === 'personnel') {
      if (item) pForm.reset(item);
      else pForm.reset({ PersonnelID: `P-${Date.now()}`, Name: '', Position: '', DepartmentID: '' });
    } else {
  ...
  const onPersonnelSubmit = async (data: any) => {
    ...
    const success = editingItem ? await updateRecord('Personnel', data) : await createRecord('Personnel', data);
  ...
  const handleDelete = async (item: any) => {
    const itemName = activeTab === 'personnel' ? item.Name : item.DepartmentName;
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-black border-b border-slate-100">
            {activeTab === 'personnel' ? (
              <tr><th className="px-6 py-4">รหัส</th><th className="px-6 py-4">ชื่อ-นามสกุล</th><th className="px-6 py-4">ตำแหน่ง</th><th className="px-6 py-4">แผนก</th><th className="px-6 py-4 text-right">จัดการ</th></tr>
            ) : (
            ...
            {isLoading ? (
              ...
              personnel.map(p => (
                <tr key={p.PersonnelID} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-500">{p.Position}</td>
                  <td className="px-6 py-4 text-slate-500 font-medium">{departments.find(d => d.DepartmentID === p.DepartmentID)?.DepartmentName || p.DepartmentID}</td>
            ...
            <div className="p-6">
              {activeTab === 'personnel' ? (
                <form onSubmit={pForm.handleSubmit(onPersonnelSubmit)} className="space-y-4">
                  <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">ชื่อ-นามสกุล</label><input {...pForm.register('Name', { required: true })} className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-primary outline-none" /></div>
                  <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">ตำแหน่ง</label><input {...pForm.register('Position')} className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-primary outline-none" /></div>
                  <div><label className="text-[10px] font-black uppercase text-slate-400 ml-1">แผนก</label><select {...pForm.register('DepartmentID', { required: true })} className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-primary outline-none appearance-none"><option value="">-- เลือกแผนก --</option>{departments.map(d => <option key={d.DepartmentID} value={d.DepartmentID}>{d.DepartmentName}</option>)}</select></div>
